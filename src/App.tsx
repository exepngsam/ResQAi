...
import { DashboardSummary, Mission, Zone, Incident, TimelineEvent } from './types';
...
export default function App() {
  ...
  const [selectedMissionForApproval, setSelectedMissionForApproval] = useState<Mission | null>(null);
  ...
  const handleApproveMission = async (missionId: string, officerName: string) => {
  ...
  const handleRejectMission = async (missionId: string, reason: string) => {
    await api.rejectMission(missionId, reason);
    if (summary) {
      const updatedMissions = summary.active_missions.map((m) =>
        m.id === missionId ? { ...m, status: 'REJECTED' as const, rejection_reason: reason } : m
      );
      setSummary({ ...summary, active_missions: updatedMissions });
    }
  };
  ...
  // Simulation Controls
  const handleStartSimulation = async (speed: number) => {
  ...
  if (!summary) {
    return (
      <div className="min-h-screen bg-command-bg flex items-center justify-center font-mono text-cyan-400">
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin" />
          <span className="text-sm font-bold tracking-wider">INITIALIZING DISASTERIQ COMMAND TELEMETRY...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-command-bg text-slate-100 flex flex-col font-sans">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Main Work Area */}
        <main className="flex-1 overflow-y-auto p-4 md:p-6 bg-[#070d16]/95">
          ...
          {currentTab === 'resources' && (
            <ResourcesPage
              rescueTeams={summary.zones ? undefined : []}
            />
          )}
