...
import { ShieldAlert, CheckCircle, XCircle, ArrowRight, UserCheck } from 'lucide-react';
...
interface ApprovalModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onApprove: (missionId: string, officerName: string) => Promise<void>;
  onReject: (missionId: string, reason: string) => Promise<void>;
}
...
export const ApprovalModal: React.FC<ApprovalModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-command-panel border-2 border-red-500/80 rounded-2xl max-w-xl w-full p-6 shadow-2xl shadow-red-950/50 text-slate-100 animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="flex items-center gap-3 border-b border-command-border pb-4">
          ...
          <div>
            ...
            <h3 className="text-lg font-bold text-white tracking-tight mt-0.5">
              Mission Dispatch: {mission.id}
            </h3>
          </div>
        </div>

        {/* AI Recommendation Summary */}
        <div className="mt-4 p-4 rounded-xl bg-command-bg border border-command-border">
          <div className="text-xs font-mono uppercase text-slate-400 font-semibold mb-2">
            AI Operational Recommendation:
          </div>
          <div className="text-base font-semibold text-white">
            Deploy <span className="text-cyan-400">{mission.team_name}</span> to{' '}
            <span className="text-red-400">{mission.zone_id}</span>
          </div>

          <div className="mt-3 p-3 rounded-lg bg-slate-900/90 border border-slate-700/80 text-xs text-slate-300 font-mono space-y-1.5">
            <div className="font-bold text-amber-400">Explainable Rationale:</div>
            <div>• {mission.reason}</div>
            <div>• Target Coordinates: [{mission.destination.join(', ')}]</div>
            <div>• Estimated Travel Time: ~{mission.estimated_eta_min} min via Route {mission.recommended_route_id}</div>
          </div>
        </div>
        ...
        {!showRejectInput ? (
          ...
          <div className="mt-4 space-y-1.5 font-mono text-xs">
            <label className="text-red-400 font-bold">Reason for Rejection / Alternative Tactic:</label>
            <textarea
              rows={2}
              value={rejectReason}
        ...
        {/* Action Buttons */}
        <div className="mt-6 flex items-center justify-end gap-3 pt-4 border-t border-command-border">
          <button
            type="button"
            onClick={onClose}
          ...
          {!showRejectInput ? (
            <>
              ...
              >
                ...
                {isSubmitting ? 'AUTHORIZING...' : 'APPROVE MISSION'}
              </button>
            </>
          ) : (
            <button
