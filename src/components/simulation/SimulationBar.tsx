...
export const SimulationBar: React.FC<SimulationBarProps> = ({
  ...
  const [selectedSpeed, setSelectedSpeed] = useState<number>(speed || 1);
  ...
  return (
    <div className="bg-command-panel border border-command-border rounded-xl p-3 px-4 flex flex-wrap items-center justify-between gap-3 shadow-xl">
      {/* Scenario Info */}
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="text-xs font-mono font-bold text-white uppercase tracking-wider flex items-center gap-2">
            <span>ODISHA FLOOD DEMO SIMULATION</span>
            <span className="text-[10px] px-2 py-0.5 rounded bg-slate-800 text-slate-400 font-normal">
              STEP {step} OF {maxSteps}
            </span>
          </div>
          <div className="text-[11px] text-slate-400 font-mono">
            {step === 0 && 'Scenario initialized at baseline flood stage.'}
            {step === 1 && 'Upstream reservoir gates opened: surge flow entering delta.'}
            {step === 2 && 'Inundation expanding across coastal lowlands.'}
            {step === 3 && 'Access Road R-17 washed out; culvert scouring verified.'}
            {step === 4 && '14 possible victims detected; autonomous reroute computed.'}
            {step === 5 && 'EOC authorizes Mission MIS-801: rescue boats en route.'}
            {step === 6 && 'Extraction underway: 10 victims secured aboard life rafts.'}
            {step >= 7 && 'Mission completed: All 14 civilians evacuated to safety.'}
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 font-mono text-xs">
        {/* Speed selectors */}
        <div className="flex items-center bg-command-bg border border-command-border rounded-lg p-0.5 mr-2">
          {[1, 2, 5].map((s) => (
            <button
              key={s}
              type="button"
