...
export const DisasterMap: React.FC<DisasterMapProps> = ({
  ...
  const [activeConfig, setActiveConfig] = useState<MapConfig>(propMapConfig || getMapConfig());
  ...
  const layerGroupsRef = useRef<{
    ...
    teams: L.LayerGroup;
    victims: L.LayerGroup;
    routes: L.LayerGroup;
  } | null>(null);

  // Sync with propMapConfig or storage updates
  useEffect(() => {
    if (propMapConfig) {
  ...
  useEffect(() => {
    ...
    // Center on Odisha Coastal Delta
    ...
    // Create dynamic feature layer groups
    const layerGroups = {
  ...
  // Hot-swap Tile Layer when Map Engine or API Key changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (layers.zones && zones) {
      zones.forEach((z) => {
        ...
        const poly = L.polygon(z.polygon as L.LatLngExpression[], {
          ...
          dashArray: z.risk_level === 'CRITICAL' ? '4, 4' : undefined,
        });
    ...
    if (layers.roads && roads) {
      roads.forEach((r) => {
        ...
        const line = L.polyline(r.coordinates as L.LatLngExpression[], {
          color,
          weight,
          dashArray,
          opacity: 0.85,
    ...
    // 6. VICTIM CLUSTERS & INCIDENTS (Pulsing critical markers)
    ...
    // 7. ACTIVE ROUTE VISUALIZATION (Animated Polyline)
    if (layers.routes && activeRoute && activeRoute.length > 1) {
      const routeLine = L.polyline(activeRoute as L.LatLngExpression[], {
        color: '#06b6d4',
        weight: 5,
        opacity: 0.9,
        lineCap: 'round',
  ...
  return (
    <div className="relative w-full h-full min-h-[400px]">
      <div ref={mapContainerRef} className="w-full h-full rounded-lg overflow-hidden border border-command-border shadow-2xl" />
    </div>
  );
};
