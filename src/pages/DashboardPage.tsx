...
import { DashboardSummary, Incident, Mission, Zone } from '../types';
...
interface DashboardPageProps {
  ...
  onOpenMissionApproval: (mission: Mission) => void;
...
export const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {pendingMission && (
        <div className="p-4 rounded-xl bg-gradient-to-r from-red-950/80 via-command-panel to-command-card border-2 border-red-500/80 shadow-2xl shadow-red-950/40 flex flex-col md:flex-row md:items-center justify-between gap-4 animate-pulse">
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 rounded-lg bg-red-600 flex items-center justify-center text-white shrink-0 mt-0.5 shadow-lg">
              <AlertOctagon className="w-5 h-5" />
            </div>
            <div>
              <div className="text-xs font-mono font-bold uppercase tracking-wider text-red-400 flex items-center gap-2">
                <span>ACTION REQUIRED: HUMAN-IN-THE-LOOP APPROVAL</span>
                <span className="px-1.5 py-0.2 rounded bg-red-900/80 text-[10px] text-white">PRIORITY 1</span>
              </div>
              <div className="text-sm font-semibold text-white mt-0.5">
                AI Recommendation: Dispatch {pendingMission.team_name} to Zone 7
              </div>
              <div className="text-xs text-slate-300 font-mono mt-1">
                Reason: {pendingMission.reason}
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-command-panel border border-command-border rounded-xl p-4 flex flex-col shadow-xl">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-command-panel border border-command-border rounded-xl p-4 shadow-xl">
            ...
            <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
              {summary.recent_alerts.map((alt) => (
                ...
                >
                  ...
                  <div className="text-xs font-semibold text-slate-200 mt-1.5">{alt.title}</div>
                  <div className="text-[11px] text-slate-400 font-mono mt-0.5 leading-snug">{alt.message}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              {summary.recent_incidents.map((inc) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-mono text-amber-400">{inc.zone_name}</span>
                  </div>
                  ...
                  <div className="text-[10px] font-mono text-slate-400 mt-1 flex items-center justify-between">
                    ...
                    <span className="text-cyan-400 uppercase font-bold">{inc.status}</span>
                  </div>
                </div>
              ))}
            </div>
