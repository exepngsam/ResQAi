...
import { Smartphone, Check, X, MapPin, Navigation, Clock, ShieldCheck } from 'lucide-react';
import { Mission } from '../types';

interface MobileResponsePageProps {
  missions: Mission[];
}

export const MobileResponsePage: React.FC<MobileResponsePageProps> = ({ missions }) => {
  const activeMission = missions.find((m) => m.status === 'APPROVED' || m.status === 'IN_TRANSIT') || missions[0];
  const [responderStatus, setResponderStatus] = useState<string>(activeMission?.status || 'IN_TRANSIT');

  return (
    <div className="max-w-md mx-auto space-y-4">
      {/* Mobile Frame Header */}
      <div className="p-3 bg-command-panel border border-command-border rounded-xl flex items-center justify-between text-xs font-mono">
        <div className="flex items-center gap-2">
          <Smartphone className="w-4 h-4 text-cyan-400" />
          <span className="font-bold text-white">RESPONDER HUD</span>
        </div>
      ...
      {activeMission ? (
        <div className="p-5 rounded-2xl bg-command-panel border border-command-border shadow-2xl space-y-4 font-mono text-xs">
          {/* Status Badge */}
          <div className="flex items-center justify-between">
            <span className="text-red-400 font-bold uppercase tracking-wider">
              {activeMission.priority} IMMEDIATE DISPATCH
            </span>
          ...
          <div>
            <h3 className="text-base font-bold text-white tracking-tight">{activeMission.incident_title}</h3>
            <div className="text-slate-400 text-xs mt-1">Zone: {activeMission.zone_id}</div>
          </div>

          {/* Telemetry Card */}
          <div className="p-3.5 rounded-xl bg-command-bg border border-command-border space-y-2">
            <div className="flex justify-between items-center text-slate-300">
              <span className="flex items-center gap-1.5 text-slate-400">
                <MapPin className="w-3.5 h-3.5 text-red-400" />
                Target Coordinates:
              </span>
              <span className="font-bold text-white">[{activeMission.destination.join(', ')}]</span>
            </div>
            <div className="flex justify-between items-center text-slate-300">
              <span className="flex items-center gap-1.5 text-slate-400">
              ...
              <span className="font-bold text-cyan-300">Canal Corridor R-18</span>
            </div>
            <div className="flex justify-between items-center text-slate-300">
              <span className="flex items-center gap-1.5 text-slate-400">
        ...
        <div className="p-8 text-center text-slate-500 font-mono text-xs">No active responder missions assigned.</div>
      )}
    </div>
  );
};
