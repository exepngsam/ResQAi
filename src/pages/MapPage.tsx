...
import { Zone, Road, Hospital, Shelter, RescueTeam, Incident } from '../types';
...
export const MapPage: React.FC<MapPageProps> = ({
  ...
  const [selectedZone, setSelectedZone] = useState<Zone | null>(zones[6] || null); // Default to Zone 7 (Erasama)
  ...
  const activeRouteCoordinates: [number, number][] = [
    [20.28, 86.20],
    [20.24, 86.28],
    [20.22, 86.34],
    [20.20, 86.39],
  ...
  return (
    <div className="relative h-[calc(100vh-8.5rem)] w-full flex flex-col lg:flex-row gap-4">
      {/* Map Viewport */}
      <div className="flex-1 h-full relative rounded-xl overflow-hidden border border-command-border">
        <DisasterMap
          zones={zones}
          roads={roads}
          hospitals={hospitals}
          ...
          incidents={incidents}
          activeRoute={layers.routes ? activeRouteCoordinates : undefined}
        ...
        <div className="absolute top-4 left-4 z-[400] flex flex-col gap-2 max-w-xs select-none">
          ...
          >
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-lg bg-cyan-950 flex items-center justify-center text-cyan-400 border border-cyan-500/40 group-hover:scale-105 transition-transform">
                <Key className="w-3.5 h-3.5" />
              </div>
              <div className="text-left font-mono">
                <div className="text-[9px] uppercase tracking-wider text-slate-400 flex items-center gap-1">
                  <span>GIS ENGINE &amp; API KEY</span>
                  {mapConfig.provider !== 'carto_dark' && (
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                  )}
                </div>
                <div className="text-xs font-bold text-cyan-300 truncate max-w-[160px]">
                  {tileDef.providerName}
                </div>
              </div>
            </div>
            <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-800 text-slate-300 border border-slate-700 group-hover:bg-cyan-900 group-hover:text-cyan-200">
              CONFIG
            </span>
          </button>

          {/* Layer Toggles Panel */}
          <div className="bg-command-panel/95 backdrop-blur-md border border-command-border rounded-xl p-3 shadow-2xl">
            <div className="text-xs font-mono font-bold uppercase text-slate-300 flex items-center gap-1.5 pb-2 border-b border-command-border mb-2">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded transition-colors text-left cursor-pointer ${
                      active ? 'bg-command-bg text-white' : 'text-slate-500 hover:text-slate-300'
                    }`}
                  >
      ...
      <div className="w-full lg:w-96 bg-command-panel border border-command-border rounded-xl p-4 flex flex-col justify-between shadow-xl overflow-y-auto">
        {selectedZone ? (
          <div>
            <div className="flex items-center justify-between pb-3 border-b border-command-border">
              <div>
                ...
                <h3 className="text-base font-bold text-white tracking-tight">{selectedZone.name}</h3>
              </div>
              <Badge level={selectedZone.risk_level} size="md" />
            </div>

            <div className="mt-4 space-y-3 font-mono text-xs">
              <div className="p-3 rounded-lg bg-command-bg border border-command-border flex items-center justify-between">
                <span className="text-slate-400">Decision-Support Risk:</span>
                <span className="text-base font-bold text-red-400">{selectedZone.risk_score} / 100</span>
              </div>

              <div className="p-3 rounded-lg bg-command-bg border border-command-border space-y-1.5">
              ...
              <div className="mt-4 p-3 rounded-lg bg-cyan-950/30 border border-cyan-500/40 text-cyan-300">
                <div className="font-bold flex items-center gap-1.5 mb-1">
                  <Navigation className="w-3.5 h-3.5" />
                  <span>Rescue Dispatch Guidance:</span>
                </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-500 font-mono text-xs">
            ...
            <span>Click any polygon on map to inspect zone telemetry</span>
          </div>
        )}

        {/* Bottom GIS API Key Engine Footer */}
        <div className="pt-3 border-t border-command-border text-[10px] font-mono text-slate-400 flex items-center justify-between">
          <div className="truncate mr-2">
            <span className="text-slate-500">GIS ENGINE: </span>
            <span className="text-cyan-400 font-bold">{tileDef.providerName}</span>
          </div>
          <button
            type="button"
            onClick={() => setIsApiKeyModalOpen(true)}
