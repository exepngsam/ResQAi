...
import { Send, MapPin, Camera, AlertTriangle, CheckCircle, ShieldAlert } from 'lucide-react';
...
export const CitizenReportPage: React.FC = () => {
  ...
  const [emergencyType, setEmergencyType] = useState<string>('FLOOD_TRAPPED');
  const [latitude, setLatitude] = useState<number>(20.194);
  const [longitude, setLongitude] = useState<number>(86.431);
  const [phone, setPhone] = useState<string>('+91 94371 88290');
  ...
  const handleGetLocation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          // Fallback to simulated Odisha coordinate
          setLatitude(20.194);
          setLongitude(86.431);
        }
      );
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {submittedResult ? (
        <div className="p-6 rounded-2xl bg-command-panel border-2 border-emerald-500/80 shadow-2xl font-mono text-xs space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-white text-base font-bold">Report Reference: {submittedResult.report_id}</div>
            </div>
          </div>

          <div className="p-4 rounded-xl bg-command-bg border border-command-border space-y-2">
            <div className="text-slate-400">Automated AI Triage Assessment:</div>
            <div className="text-white">
              Category: <strong className="text-cyan-400">{submittedResult.ai_category}</strong>
            </div>
            <div className="text-white">
              Initial Severity: <strong className="text-red-400">{submittedResult.ai_severity}</strong>
            </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 rounded-2xl bg-command-panel border border-command-border shadow-xl space-y-4 font-mono text-xs">
          <div>
            <label className="text-slate-300 block mb-1 font-bold">Situation Description / Nature of Emergency:</label>
            <textarea
              rows={3}
              required
              value={description}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-slate-300 block mb-1 font-bold">Number of People Stranded:</label>
              <input
                type="number"
                min={1}
                max={500}
            ...
            <div>
              <label className="text-slate-300 block mb-1 font-bold">Emergency Category:</label>
              <select
                value={emergencyType}
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-slate-300 font-bold">GPS Geolocation Coordinates:</label>
              <button
                type="button"
          ...
          <div>
            <label className="text-slate-300 block mb-1 font-bold">Contact Mobile / Satellite Phone:</label>
            <input
              type="text"
              value={phone}
          ...
          <div className="p-3 rounded-lg bg-amber-950/20 border border-amber-500/40 text-amber-300 text-[11px] leading-snug">
          ...
          >
            ...
            {isSubmitting ? 'TRANSMITTING SOS...' : 'TRANSMIT EMERGENCY SOS REPORT'}
          </button>
        </form>
      )}
    </div>
