...
import { PlayCircle, RotateCcw, FastForward, Activity, CheckCircle, ArrowRight } from 'lucide-react';
...
export const SimulationPage: React.FC<SimulationPageProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-2xl bg-command-panel border border-command-border shadow-xl">
        <h3 className="text-base font-bold text-white uppercase tracking-wider font-mono flex items-center gap-2">
          <PlayCircle className="w-5 h-5 text-amber-400" />
          <span>3-Minute SIH Demo Orchestrator (Deterministic Script)</span>
        </h3>
              ...
              <div
                ...
                className={`p-4 rounded-xl border transition-all ${
                  isCurrent
                    ...
                    : isCompleted
                    ? 'bg-command-bg/40 border-emerald-500/40 text-slate-300'
                    : 'bg-command-bg border-command-border text-slate-500'
                }`}
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className={`font-bold text-sm ${isCurrent ? 'text-amber-400' : isCompleted ? 'text-white' : 'text-slate-400'}`}>
                        {scene.title}
                      </div>
                      <div className="text-slate-300 mt-0.5 text-xs">{scene.desc}</div>
                    </div>
                  </div>
                ...
                <div className="mt-2 pl-10 text-[11px] text-slate-400">
                  <strong className="text-slate-300">Action:</strong> {scene.action}
                </div>
              </div>
            );
          })}
