...
export const SettingsPage: React.FC = () => {
  ...
  const [aiMode, setAiMode] = useState<'DEMO_SIMULATION' | 'REAL_AI'>('DEMO_SIMULATION');
  const [savedSuccess, setSavedSuccess] = useState(false);

  // GIS Map API Key States
  ...
  const handleResetCarto = () => {
    const reset = { ...mapConfig, provider: 'carto_dark' as MapTileProvider };
    setMapConfig(reset);
    saveMapConfig(reset);
    setGisSavedSuccess(true);
    setTimeout(() => setGisSavedSuccess(false), 2000);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="p-6 rounded-2xl bg-command-panel border border-cyan-500/30 space-y-5 font-mono text-xs shadow-xl">
        ...
        <div className="space-y-2">
          <label className="text-[11px] font-bold text-slate-300 uppercase">Active Tile Provider Engine:</label>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            <button
              ...
              className={`p-3 rounded-xl border text-left cursor-pointer transition-all ${
                mapConfig.provider === 'carto_dark'
                  ...
                  : 'bg-command-bg border-command-border text-slate-400'
              }`}
            >
              <div className="font-bold text-slate-200">CartoDB Dark</div>
            ...
            >
              ...
              <div className="text-[10px] text-blue-400 mt-0.5">High-Res Satellite</div>
            </button>
            ...
            >
              <div className="font-bold text-slate-200">Google Maps</div>
              <div className="text-[10px] text-amber-400 mt-0.5">Hybrid Satellite + Road</div>
            </button>
          </div>
        </div>

        {/* Mapbox Token Input */}
        <div className="space-y-1.5 p-3.5 rounded-xl bg-command-bg border border-command-border">
          <div className="flex justify-between items-center text-slate-300">
            <span className="font-bold">Mapbox Access Token (VITE_MAPBOX_TOKEN):</span>
            <a
        ...
        <div className="space-y-1.5 p-3.5 rounded-xl bg-command-bg border border-command-border">
          <div className="flex justify-between items-center text-slate-300">
            <span className="font-bold">Google Maps Platform API Key (VITE_GOOGLE_MAPS_API_KEY):</span>
            <a
        ...
        <div className="space-y-1.5 p-3.5 rounded-xl bg-command-bg border border-command-border">
          <div className="flex justify-between items-center text-slate-300">
            <span className="font-bold">OpenWeather Radar API Key (VITE_OPENWEATHER_API_KEY):</span>
            <label className="flex items-center gap-1.5 cursor-pointer text-slate-400 text-[10px]">
              <input
                type="checkbox"
        ...
        <div className="pt-2 flex items-center justify-between">
          ...
          >
            ...
            <span>Reset to Free CartoDB</span>
          </button>
      ...
      <div className="p-6 rounded-2xl bg-command-panel border border-command-border space-y-4 font-mono text-xs shadow-xl">
        <div className="flex items-center justify-between pb-3 border-b border-command-border">
          <div className="flex items-center gap-2 text-sm font-bold text-white uppercase">
            <Sliders className="w-4 h-4 text-cyan-400" />
            <span>Configurable Risk Engine Weights</span>
          </div>
          <span className="text-[10px] text-slate-400">Total: {Object.values(weights).reduce((a, b) => a + b, 0)}%</span>
        </div>
          ...
          {[
            ...
            <div key={item.key} className="space-y-1">
              <div className="flex justify-between text-slate-300">
                ...
                <span className="font-bold text-cyan-400">{weights[item.key as keyof typeof weights]}%</span>
              </div>
              <input
                type="range"
                min={5}
        ...
        <div className="pt-2 flex justify-end">
          ...
          >
            {savedSuccess ? 'WEIGHTS APPLIED' : 'APPLY CONFIGURATION'}
          </button>
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-command-panel border border-command-border space-y-4 font-mono text-xs shadow-xl">
        <div className="text-sm font-bold text-white uppercase flex items-center gap-2">
          ...
          <span>AI Inference Architecture Mode</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className={`p-4 rounded-xl border text-left cursor-pointer transition-all ${
              aiMode === 'DEMO_SIMULATION'
                ? 'bg-amber-950/30 border-amber-500 text-white shadow-lg'
                : 'bg-command-bg border-command-border text-slate-400'
            }`}
          >
            <div className="font-bold text-sm text-amber-400">DEMO SIMULATION (Active)</div>
          ...
          >
            <div className="font-bold text-sm text-cyan-400">REAL AI MODE</div>
            <div className="text-[11px] text-slate-400 mt-1">
      ...
      <div className="p-6 rounded-2xl bg-command-panel border border-command-border space-y-3 font-mono text-xs shadow-xl">
        <div className="text-sm font-bold text-white uppercase flex items-center gap-2 pb-2 border-b border-command-border">
          <Server className="w-4 h-4 text-slate-400" />
          <span>Offline Resilience &amp; Observability</span>
        </div>

        <div className="space-y-2 text-slate-300">
          <div className="flex justify-between p-2.5 bg-command-bg rounded-lg border border-command-border">
          ...
          <div className="flex justify-between p-2.5 bg-command-bg rounded-lg border border-command-border">
            <span>WebSockets Reconnect State:</span>
